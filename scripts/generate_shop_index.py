...
def display_menu(shops, indexed_urls=None):
	...
	if choice == "1":
		...
		if 'cookies' not in GLOBAL_CONFIG: GLOBAL_CONFIG['cookies'] = {}
		if domain not in GLOBAL_CONFIG['cookies']: GLOBAL_CONFIG['cookies'][domain] = {}
				...
				for part in raw_cookies.split(';'):
					if '=' in part:
						try:
							...
							GLOBAL_CONFIG['cookies'][domain][k] = v
						except: pass
			else:
				GLOBAL_CONFIG['cookies'][domain]['cf_clearance'] = raw_cookies.strip()

		if user_agent:
			GLOBAL_CONFIG['cookies'][domain]['User-Agent'] = user_agent.strip()
